...
/**
 * Load credentials: from environment variables first, then frontend-saved config, then file.
 */
function loadCredentials() {
  ...
  if (process.env.GOOGLE_CREDENTIALS_JSON) {
    try {
      ...
      console.error('Error parsing GOOGLE_CREDENTIALS_JSON from env:', err);
    }
  }

  // Priority 2: Check frontend-saved config
  ...
  // Priority 3: Check credentials path from env
  if (process.env.GOOGLE_CREDENTIALS_PATH) {
    const credPath = path.isAbsolute(process.env.GOOGLE_CREDENTIALS_PATH) 
      ? process.env.GOOGLE_CREDENTIALS_PATH 
      : path.join(process.cwd(), process.env.GOOGLE_CREDENTIALS_PATH);
  ...
  // Priority 4: Check credentials path from config
  ...
  throw new Error(`Credentials not found. Set GOOGLE_CREDENTIALS_JSON or GOOGLE_CREDENTIALS_PATH in environment variables, frontend config, or .env file`);
}

/**
 * Load token: from environment variables first, then frontend-saved config, then token.json file.
 */
function loadToken() {
  ...
  if (process.env.GOOGLE_TOKEN_JSON) {
    try {
      ...
      console.error('Error parsing GOOGLE_TOKEN_JSON from env:', err);
    }
  }
  ...
  // Priority 3: Fallback to token.json file
